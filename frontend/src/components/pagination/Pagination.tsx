...
export const Pagination = ({
  ...
  const paginationFrom =
    currentPage > PAGINATION_OFFSET + 2 ? currentPage - PAGINATION_OFFSET : 1
  const paginationTo =
    currentPage + PAGINATION_OFFSET + 2 < totalPages
      ? currentPage + PAGINATION_OFFSET - 1
      : totalPages
  const paginationPages = Array.from(
    { length: paginationTo - paginationFrom + 2 },
    (_, i) => i + paginationFrom
  )

  const PaginationLink = (props: Omit<PageLinkProps, 'href'>) => (
    <PageLink {...props} href={href} />
  )
